...
导出模型为 ONNX、用 ONNX Runtime 推理（可选依赖：`pip install -e ".[deploy]"`）：

```bash
# 导出小模型（仅需 onnx，无 torch）
opl export -o model_tiny.onnx --model tiny

# 导出深度模型 MiDaS（需 torch + onnx）
opl export -o depth.onnx --model depth

# 运行 ONNX 推理（需 onnxruntime）
opl infer model_tiny.onnx
```
...
- **Stage 2（已缓解）**：视觉 SLAM
  - **能力**：实时 VO + 局部 BA + 轨迹/点云 + 地图保存与加载 + **回环检测**（关键帧描述子匹配，检测到则打 log）。
  - **缓解**：单目尺度可通过融合阶段与 LiDAR 尺度对齐（`fusion.scale_align`）；回环检测已接入，后端位姿图校正可后续接 GTSAM。
- **Stage 3（已缓解）**：LiDAR SLAM
  - **能力**：PCD 序列、ICP 建图、轨迹保存。
  - **缓解**：IMU 可在融合/VIO 中用于传播（`fusion.imu_propagation` + VIO）。
- **Stage 4（已缓解）**：多传感器融合
  - **能力**：2D/ **3D** pose graph（`fusion.pose_graph_3d`）、LiDAR-Vision 融合、**VIO（含 IMU 传播）**、尺度对齐。
  - **缓解**：3D 位姿图（仅平移优化）、IMU 一阶积分 + VIO 融合。
- **Stage 5（已开始）**：3D 重建（SfM、NeRF/3DGS 占位）
  - **能力**：增量式 SfM、PLY/位姿输出。
  - **限制**：稀疏重建、稠密/NeRF/3DGS 待接入。
- **Stage 6（已开始）**：模型压缩与端侧部署
  - **能力**：ONNX 导出（tiny/depth）、ONNX Runtime 推理、TensorRT 占位。
  - **可选依赖**：`pip install -e ".[deploy]"`（onnx, onnxruntime, torch）。
...
deployment/           # 端侧部署（ONNX 导出与推理、TensorRT 占位）
